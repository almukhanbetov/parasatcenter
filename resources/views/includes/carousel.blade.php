<div class="container-fluid p-0 pb-5 wow fadeIn" data-wow-delay="0.1s">
    <div class="owl-carousel header-carousel position-relative">
        <div class="owl-carousel-item position-relative" data-dot="<img src='{{asset('/site/img/carousel-1.jpg')}}'>">
            <img class="img-fluid" src="{{asset('/site/img/carousel-1.jpg')}}" alt="">
            <div class="owl-carousel-inner">
                <div class="container">
                    <div class="row justify-content-start">
                        <div class="col-10 col-lg-8">
                            <h1 class="display-2 text-white animated slideInDown">Обучаем. Развиваем.</h1>
                            <h1 class="display-2 text-white animated slideInDown">Повышаем</h1>
                            <p class="fs-5 fw-medium text-white mb-4 pb-3">
                                Квалификации специалистов, педагогов и преподавателей вузов.
                            </p>
                            <a href="tel:+77771005123" class="btn btn-primary rounded-pill py-3 px-5 animated slideInLeft">Связаться</a>
                        </div>
                    </div>
                </div>
            </div>
        </div>

    </div>
</div>
